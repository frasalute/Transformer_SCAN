...
def analyze_per_length(results, data_loader, model, hyperparams):
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Final Evaluation"):
            ...
            # Use greedy decoding (handled by train's functions)
            pred = train.greedy_decode(
                model,
                src,
                max_len=tgt.size(1),
                start_symbol=data_loader.dataset.vocab.special_tokens["<BOS>"],
                end_symbol=data_loader.dataset.vocab.special_tokens["<EOS>"],
            ...
            for i in range(src.size(0)):
                ...
                _, seq_acc = train.calculate_accuracy(pred[i].unsqueeze(0), tgt[i].unsqueeze(0), pad_idx)
...
def run_experiment():
    """Run Experiment 2 using the new training module."""
    ...
    model, _, _ = train(
        ...
        test_path=test_path,  # Full test dataset used for validation
